...
class GetTechniqueQuizUseCase @Inject constructor(
    private val quizRepository: QuizRepository
) {
    suspend operator fun invoke(genreId: Long): Flow<Response<TechniqueQuizList>> = flow {
        try {
            emit(Response.Loading)
            emit(Response.Success(data = quizRepository.getAllTechniqueArticle(genreId)))
        } catch (e: IOException) {
